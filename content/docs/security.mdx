...
The daemon only executes typed commands from a known allowlist. It cannot be instructed to run arbitrary shell commands. Full details: [Security & trust model](/machines/security).
...
- `mantlerd` includes an audit log of every server-originated command on your machine (type, outcome, denial reason). See [Audit log](/machines/security#audit-log).
